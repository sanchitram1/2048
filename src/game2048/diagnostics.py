...
def _parse_compare_args() -> argparse.Namespace:
    """Parse args for 'diagnose compare' subcommand."""
    parser = argparse.ArgumentParser(
...
def compare_main() -> None:
    """Entry point for 'diagnose compare' subcommand."""
    ...
    try:
        ...
        if ":" not in source_spec:
            raise ValueError(
                f"Invalid source spec: {source_spec}. Use rollout:<path.pt> or npz:<path.npz>"
            )
        ...
        if source_kind == "rollout":
            ...
            print(
                f"[compare] Collecting rollout from {source_path} ({args.episodes} episodes, {args.tail_moves} tail moves)..."
            )
            ...
            rollout_npz_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        manifest = {
            ...
            "source": {
                ...
                "rollout_boards_npz": str(rollout_npz_path)
                if source_kind == "rollout"
                else None,
                "episodes": args.episodes,
                "tail_moves": args.tail_moves,
